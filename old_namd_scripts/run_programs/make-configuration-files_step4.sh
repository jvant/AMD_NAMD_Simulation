...
do
    ...
    # AMD configuration scripts
    for i in {31..45}
    do 
	cat ../../scripts/amd_template.inp > step$i\_equilibration.inp
     	sed -i s/"STEPNUM"/"$i"/ step$i\_equilibration.inp
     	sed -i s/FILENAME/${name:0:4}-solv_ion/ step$i\_equilibration.inp
    done
